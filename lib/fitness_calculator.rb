...
class FitnessCalculator
  ...
  def examine_possible_moves(board)
    ...
    available_spaces.each do |space|
      evaluate_fitness_of_space(space, board)
    end
  end

  def evaluate_fitness_of_space(space, board, original_move = space, depth = 0)
    future_board = board.dup
    if computer_can_win?(future_board, space)
      increase_fitness_for_winning(original_move, depth)
    elsif tie?(future_board)
      fitness[original_move] = 0
    else
      future_board[space.to_i] = current_player.symbol
      dig_deeper_into_tree(future_board, original_move, depth)
    end
  end
  ...
  def increase_fitness_for_winning(original_move, depth)
    # priortize_this_move(original_move) if depth == 0
    fitness[original_move] += (10 + -depth)
  end

  def decrease_fitness_for_losing(original_move, depth)
    # priortize_this_move(original_move) if depth == 0
    fitness[original_move] -= (10 + -depth)
  end
  ...
  def dig_deeper_into_tree(future_board, original_move, depth)
    enemy_moves = calcuate_available_spaces(future_board)
    enemy_moves.each do |move|
      if opponent_can_win?(future_board, move)
        decrease_fitness_for_losing(original_move, depth)
      elsif tie?(future_board)
        fitness[original_move] = 0
      else
        future_board[move.to_i] = opponent.symbol
        p future_board
        available_spaces = calcuate_available_spaces(future_board)
        p available_spaces
        available_spaces.each do |space|
            evaluate_fitness_of_space(space, future_board, original_move, depth+1)
        end
      end
    end
  end
